...
def add_files():
    ...
    root.quit()
    if filename:
        if os.path.splitext(filename)[1].lower() in ['.mp3', '.wav', '.ogg']:
            if filename not in data["songs"]:
...
def add_folder():
    ...
    for filename in os.listdir(folder):
        if filename.endswith(('.mp3', '.wav', '.ogg')):
            ...
            if filepath not in data["songs"]:
                update_database(filepath)
                dpg.add_button(label=f"{ntpath.basename(filename)}", callback=play, width=-1, height=25,
                               user_data=filepath.replace("\\", "/"), parent="list")
                dpg.add_spacer(height=2, parent="list")
...
def search(sender, app_data, user_data):
    ...
    for song in songs:
        if app_data.lower() in song.lower():
            dpg.add_button(label=f"{ntpath.basename(song)}", callback=play, width=-1, height=25, user_data=song,
                           parent="list")
            dpg.add_spacer(height=2, parent="list")
        ...
        with dpg.child_window(autosize_x=True, border=False):
            with dpg.child_window(autosize_x=True, height=80, no_scrollbar=True):
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_slider_float(tag="volume", width=120, height=15, pos=(10, 59), format="%.0f%.0%",
